...
/*    if (imgSmall != null)
    {
      if (splines == null || splines.length != imgSmall.height )
      {
        splines = new Spline3D[imgSmall.height];    
        for (int j=0; j<splines.length; j++)
        {
          splines[j] = new Spline3D();
          for (int i=0;i<imgSmall.width;i++)
            splines[j].add(i, j, 0.0); // temp, just to add points here
        }
      }
    }
*/  }
  ...
  // drawSplines
  // ------------------------------------------------------
  void drawSplines(PGraphics g, PImage img, float hh)
  {
/*      float sc = 3*s;
    if (splines != null)
    {
      ...
      for (int j=0;j<h;j++)
      {
        points = splines[j].getPointList();
        for (int i=0;i<w;i++)
        {
          p = img.pixels[i+j*w] & 0x000000FF;

          points.get(i).set( i*sc, j*sc, float(p)/255.0*h*3 );
        }
      }
    ...
    g.pushMatrix();
//    g.scale(3);
    g.translate(-sc*img.width/2, -sc*img.height/2);
    for (int j=0; j<splines.length; j++)
    {
      List<Vec3D> vertices = splines[j].computeVertices(10);
      List<Vec3D> vertices2 = splines[j].getDecimatedVertices(8);
     // beginShape(POINTS);
      for (Iterator i=vertices2.iterator(); i.hasNext(); ) {
